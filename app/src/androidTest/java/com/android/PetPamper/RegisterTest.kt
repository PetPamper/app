...
class RegisterTest : TestCase() {
  ...
  fun testGroomerRegister() = run {
    step("Attempt to register as groomer") {
      ComposeScreen.onComposeScreen<MainScreen>(composeTestRule) {
        groomerToggle {
          assertExists("Groomer toggle doesn't exist")
          assertIsDisplayed()
    ...
    ComposeScreen.onComposeScreen<GroomerRegisterScreen>(composeTestRule) {
      fun testSimpleLayout(lblTxt: String, inTxt: String) {
        displayText {
          assertExists("Display text doesn't exist")
          assertIsDisplayed()
          assertTextEquals(lblTxt)
        }
        inputText {
          assertExists("Input text box doesn't exist")
          assertIsDisplayed()
          performTextInput(inTxt)
        }
        arrowButton {
          assertExists("Arrow button doesn't exist")
          assertIsDisplayed()
          performClick()
        }
      }
