...
class FirebaseConnection : Database() {
    ...
    fun addUser(user: User, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
        storeData("users",user.email,user)
        /*db.collection("users")
            .document(user.email)
            .set(user)
    ...
    fun getUserData(uuid: String): DocumentReference {
    ...
    fun getUserUidByEmail(email: String): Task<QuerySnapshot> {
        ...
        query.get().addOnCompleteListener{task ->
            if(task.isSuccessful){
                ...
                Log.d("TAG_getUUIDbyEmail_doc",docs.toString())
            }
        }
        Log.d("TAG_getUUIDbyEmail_query_app_name",query.firestore.app.name)
        return query.get()
    }
