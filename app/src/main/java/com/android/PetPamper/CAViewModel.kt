...
class CAViewModel
@Inject
constructor(val auth: FirebaseAuth, val db: FirebaseFirestore, val storage: FirebaseStorage) :
    ViewModel() {
  ...
  private fun getUserData(email: String) {
    ...
    db.collection(COLLECTION_USER).whereEqualTo("email", email).addSnapshotListener { value, error
      ->
      if (error != null) {
  ...
  private fun populateChats() {
    ...
    db.collection(COLLECTION_CHAT)
        ...
        .addSnapshotListener { value, error ->
          ...
          if (value != null) chats.value = value.documents.mapNotNull { it.toObject<ChatData>() }
                    ...
                    if (it.isEmpty)
                      ...
                      val chat =
                          ChatData(
                              ...
                              ChatUser(
                                  chatPartner.userId, chatPartner.name, chatPartner.phoneNumber))
