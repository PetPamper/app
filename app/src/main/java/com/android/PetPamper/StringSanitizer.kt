...
object StringSanitizer{
  fun sanitizeEmail(str:String):String{
    val cleanedEmail = str.replace(Regex("""[^a-zA-Z0-9\.\!\#\$\%\&\'\*\+\-\/\=\?\^\_\`\{\|\}\~\@]"""),"")
    val sanitizedEmail = cleanedEmail.replace(Regex("""(?<email>${RePatterns().AUTOLINK_EMAIL_ADDRESS.pattern()})(?<rest>.*)"""),
        {matchRes ->
  ...
  fun sanitizePhone(str:String):String{
    val cleanedPhone = if(str.startsWith("+")){str.replaceFirst("+","00")}else{str}
  ...
  fun sanitizeInt(str:String):String{
    val cleanedNumber = str.replace(Regex("""[,'.]"""),",").reversed().replaceFirst(",",".").reversed()
